...
s2.FaceColor = 'y';
legend('True Conc. B', 'Pred. Conc. B', 'Interpreter','latex', 'FontSize', 12)
title('Variation of Conc. of B across space and time in a PFR', 'Interpreter','latex', 'FontSize', 20)
...
zlabel('Dimensionless concentration ($C_{B}^{\ast}$)', 'Interpreter','latex', 'FontSize',14)
ax = gca;
...
function [c, f, s] = pdefun(x, t, u, dudx) % Equation to solve
    ...
    s = [8.135 - 5.19058 * u(3) + 1.8509 * u(2) + 0.3642 * u(1) * dudx(2);
         -9.6913 + 7.5306 * u(3) - 2.8265 * u(2) - 2.4402 * dudx(2);
         1.8083 - 2.1424 * dudx(3) - 1.05926 * u(3) + 0.5024 * u(2)];  
end
