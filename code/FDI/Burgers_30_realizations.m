num_realizations = 30;
Qval = zeros(40,num_realizations,11) ; 
termsfull = strings(40,num_realizations,11) ; 
valuesfull = zeros(40,num_realizations,11) ; 
snrs = [20000,15000,10000,7500,5000,3000,2000,1000,800,500,300,200,100,90,80,70,60,50,40,30,20,10,8,6,5,4,3,2,1,0.9,0.8,0.75,0.7,0.6,0.5,0.4,0.3,0.25,0.2,0.1];

% ================ Importing the dataset =======================
fname = '..\..\Datasets\2_'+string(0)+'.pkl';
...
for ni=1:1:40
    for nj=1:1:num_realizations
        % ================ Adding noise ===============================
        un = u + normrnd(0,sqrt(var(u,0,'all')./snrs(ni)),size(u,1),size(u,2));


        % ================= Building the library =======================
        U_dxn=differfitx(un,[-15:1:15],dx,10,4);
        U_dtn=differfitx(un.',[-15:1:15],dt,7,2);
        U=U_dxn(:,:,1);
        U2=U.^2;
        Ut1=U_dtn(:,:,2).';
        Ux1=U_dxn(:,:,2);
       ...
       % ================= Finding Q ===============================
        ...
        termsfull(ni,nj,:)=terms_sorted;
        valuesfull(ni,nj,:)=values(terms_sorted_idx);
    end
end

rmses = zeros(40,num_realizations) ; 
actual= zeros(11,1);
actual(1)= -1;
actual(5)= 0.1;
for ni=1:1:40
    for nj=1:1:num_realizations
        pred = valuesfull(ni,nj,:);
        rmses(ni,nj) = rms(pred,actual);
    end
end
rmses = mean(rmses,2);
...
function er=rms(pred,actual)
    pred = reshape(pred,1,[]);
    actual = reshape(actual,1,[]);
    diff = sum((pred - actual).^2);
    er = sqrt(diff/size(pred,1));
end
